...
const Tasks: React.FC = () => {
  ...
  const fetchTasks = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await leadService.getTasks({ page, limit: 10 });
      setTasks(res.data);
      setTotal(res.total);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h4 className="page-title text-xl font-bold text-gray-700 m-0">Task List</h4>
      ...
      <div className="grid grid-cols-12 gap-6 h-[calc(100vh-220px)]">
          ...
          <div className="col-span-12 lg:col-span-4 bg-white border border-gray-100 shadow-sm flex flex-col overflow-hidden">
                <div className="p-3 bg-[#f8f9fa] border-b border-gray-100 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-gray-400 uppercase">Tasks Queue</span>
                  ...
                  <div className="flex flex-col h-full">
                      <div className="p-6 border-b border-gray-100">
                          <div className="flex justify-between items-start gap-4">
                              ...
                              <div className="flex gap-2">
                                  <button 
                                    onClick={() => { setEditingTask(selectedTask); setIsModalOpen(true); }}
                                    className="bg-gray-100 text-gray-700 px-3 py-1.5 rounded text-[10px] font-bold uppercase border border-gray-200"
                                  >
                                    Edit
                                  </button>
                                  <button className="bg-brand text-white px-3 py-1.5 rounded text-[10px] font-bold uppercase">Complete</button>
                              </div>
                          </div>
                      </div>
